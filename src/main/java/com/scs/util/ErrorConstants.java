package com.scs.util;

/**
 * Author : Sanal Samuel
 * 
 */

public class ErrorConstants {

	public static final String INVALIDDATA = "INVALIDDATA";
	public static final String INVALIDREQUEST = "INVALIDREQUEST";
	public static final String SERVICEEXCEPTION = "SERVICEEXCEPTION";
	public static final String SERVICE_UNAVAILABLE = "SERVICE_UNAVAILABLE";
	public static final String USER_LOGIN_EXCEPTION = "USERLOGINEXCEPTION";
	public static final String USER_NOT_FOUND_EXCEPTION = "USERNOTFOUNDEXCEPTION";
	public static final String PLEASE_PROVIDE_EMAIL = "PLEASE_PROVIDE_EMAIL";
	public static final String SERVICEFAILURE = "SERVICEFAILURE";
	public static final String UNIQUECONSTRAINTSTATE = "23505";

	private ErrorConstants() {
	}

}
